...
model = T5ForConditionalGeneration.from_pretrained(path_common.model.value + '\\' + MODEL['name'] + "\\AClass")
model.to('cpu')
...
for index, row in tqdm(test_df.iterrows(), leave=False, total=len(result_df)):
    ...
    out_ids = model.generate(input_ids=sentence_ids['input_ids'], attention_mask=sentence_ids['attention_mask'], do_sample=False)
...
result_df.to_csv(path_or_buf='../Result/AClass/test_identity_result.csv', mode='w', index=False)
