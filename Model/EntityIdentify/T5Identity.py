...
#model = T5ForConditionalGeneration.from_pretrained(path_common.model.value + '\\' + MODEL['name'] + "\\BAL")
model = T5ForConditionalGeneration.from_pretrained('t5-small')
preModel = T5ForConditionalGeneration.from_pretrained(path_common.model.value + '\\' + MODEL['name'] + "\\BAL")
model.to('cpu')
...
for index, row in tqdm(test_df.iterrows(), leave=False, total=len(result_df)):
    ...
    path_name = "../Result/T5-Small/" + str(row['sentence_index']) + "_test.csv"
...
result_df.to_csv(path_or_buf='../Result/T5-Small/test_identity_result.csv', mode='w', index=False)
