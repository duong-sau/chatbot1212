...
model = T5ForConditionalGeneration(path_common.model.value + '\\' + MODEL['name'] + "\\T5STS-POS")
model.to('cpu')
tokenizer = T5Tokenizer.from_pretrained(MODEL['name'])
tokenConfig(tokenizer=tokenizer)

test_df = pd.read_csv(
    "https://raw.githubusercontent.com/duong-sau/chatbot1212/master/Model/Data/IntentClassification/POS/test.csv",
    header=0)
...
result_df.to_csv(path_or_buf='test_identity_result.csv', mode='w')
